...
?>
        ...
        <div class="portal-inner-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 35px; padding: 25px; border-radius: 12px; border: 1px solid var(--border-color);">
            <div>
                <h1 style="font-size: 28px; font-weight: 800; color: var(--dark-color); margin-bottom: 5px;">My Certificates</h1>
                <p style="color: var(--gray-color); font-size: 14px;">You have earned <strong style="color: var(--dark-color);"><?php echo count($completed_courses); ?></strong> certificates.</p>
            </div>
        </div>

        <div style="display: grid; grid-template-columns: repeat(auto-fill, minmax(300px, 1fr)); gap: 25px;">
            <?php if (empty($completed_courses)): ?>
                <div style="grid-column: 1/-1; text-align: center; padding: 80px; background: var(--bg-card); border-radius: 12px; border: 1px solid var(--border-color);">
                    <i class="fa fa-award" style="font-size: 60px; color: var(--border-color); margin-bottom: 20px;"></i>
                    <h3 style="font-size: 20px; font-weight: 700; color: var(--dark-color); margin-bottom: 10px;">No certificates yet</h3>
                    <p style="color: var(--gray-color); margin-bottom: 25px;">Complete 100% of a course to earn your first certificate.</p>
                    <a href="courses.php" class="btn btn-primary">Continue Learning</a>
                </div>
            <?php
else: ?>
                <?php foreach ($completed_courses as $course): ?>
                    <div style="background: var(--white); border: 1px solid var(--border-color); border-radius: 8px; overflow: hidden; display: flex; flex-direction: column;">
                        ...
                        <div style="padding: 20px; flex: 1; display: flex; flex-direction: column;">
                            <h3 style="font-size: 16px; font-weight: 700; color: var(--dark-color); margin-bottom: 8px; line-height: 1.4;"><?php echo htmlspecialchars($course['title']); ?></h3>
                            <p style="font-size: 13px; color: var(--gray-color); margin-bottom: 15px;">Instructor: <?php echo htmlspecialchars($course['instructor_name']); ?></p>
